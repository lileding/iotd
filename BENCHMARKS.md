...
Based on Criterion benchmarks with `RUST_LOG=error` (to reduce logging overhead):

### Memory Footprint
- **Base server**: ~9-11 MB
- **Per connection (idle)**: ~6-10 KB
- **1000 connections**: ~6 KB/conn (first allocation), stable after warmup

### Throughput (Criterion, single instance)
- **1→1 small (64B)**: 220,000-445,000 msg/sec
- **1→10 fan-out (64B)**: 93,000-95,000 msg/sec
- **10→10 many-to-many (64B)**: 57,000-65,000 msg/sec

### Connection Rate
- **Establishment**: ~21,000-22,500 connections/sec
...
| Scenario | Messages | Rate |
|---|---|---|
| 1pub → 1sub, 64B | 2,220,000 in 5s | **~444,000 msg/sec** |
| 1pub → 10sub, 64B | 475,000 in 5s | **~95,000 msg/sec** |
| 10pub → 10sub, 64B | 635,000 in 10s | **~63,000 msg/sec** |
| Connection rate | 16,359 in 0.73s | **~22,000 conn/sec** |

Memory footprint (first allocation):
| Connections | Growth | Per-connection |
|---|---|---|
| 100 | 1.5 MB | 15.5 KB |
| 500 | 4.7 MB | 9.6 KB |
| 1000 | 5.9 MB | 6.0 KB |

### Load Test Results (historical, via `examples/load_test.rs`)
...
Note: Load test numbers are from an earlier architecture (spawn-per-session). Criterion numbers reflect the current structured-concurrency architecture (FuturesUnordered, no spawn).
