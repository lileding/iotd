...
async fn test_qos1_subscriber_flow() {
    ...
    let received_packet_id = ((payload[packet_id_offset] as u16) << 8) | (payload[packet_id_offset + 1] as u16);
...
async fn test_qos1_message_tracking() {
    ...
    let puback = [0x40, 0x02, (packet_id1 >> 8) as u8, (packet_id1 & 0xFF) as u8];
    ...
    assert_eq!(received_ids[0], 1, "First subscriber should get packet_id=1");
    assert_eq!(received_ids[1], 1, "Second subscriber should get packet_id=1");
    
    // Now sub2 sends delayed PUBACK
    let puback = [0x40, 0x02, (packet_id2 >> 8) as u8, (packet_id2 & 0xFF) as u8];
...
async fn test_qos1_retransmission() {
    ...
    info!("Received retransmission with packet_id={} and DUP flag", retrans_packet_id);
    ...
    let result = tokio::time::timeout(Duration::from_millis(500), sub.read_exact(&mut buffer)).await;
    assert!(result.is_err(), "Should not receive any more messages after PUBACK");
...
async fn test_qos1_duplicate_handling() {
    ...
    let mut buffer = vec![0u8; 2];
    let result = tokio::time::timeout(Duration::from_millis(500), sub.read_exact(&mut buffer)).await;
    assert!(result.is_err(), "Subscriber should not receive duplicate message");
...
async fn test_qos1_max_retransmission_limit() {
    ...
    sub.read_exact(&mut vec![0u8; header[1] as usize]).await.unwrap();
    ...
    let mut buffer = vec![0u8; 2];
    let result = tokio::time::timeout(Duration::from_millis(1000), sub.read_exact(&mut buffer)).await;
    assert!(result.is_err(), "Should not receive any more messages after exceeding retry limit");
...
#[tokio::test]
async fn test_qos1_exponential_backoff() {
    use tracing::info;
    init_test_logging();
    // Start server with exponential backoff configuration
    let mut config = iotd::config::Config::default();
    config.server.address = "127.0.0.1:0".to_string();
    config.server.retransmission_interval_ms = 1000; // 1 second base
    config.server.retransmission_backoff_multiplier = 2.0; // Double each time
    config.server.retransmission_max_interval_ms = 10000; // 10 second max
    config.server.max_retransmission_limit = 5;
    let server = iotd::server::start(config).await.unwrap();
    let address = server.address().await.unwrap();
    ...
    // DO NOT send PUBACK - test exponential backoff
    info!("Not sending PUBACK, testing exponential backoff...");
    
    // Measure retry intervals
    let start = tokio::time::Instant::now();
    
    // First retry should be around 1-2 seconds
    sub.read_exact(&mut header).await.unwrap();
    assert_eq!(header[0], 0x3A); // PUBLISH with DUP flag
    sub.read_exact(&mut vec![0u8; header[1] as usize]).await.unwrap();
    let first_retry = start.elapsed();
    info!("First retry after: {:?}", first_retry);
    assert!(first_retry >= Duration::from_millis(800) && first_retry < Duration::from_millis(2000));
    
    // Second retry should be around 3-5 seconds (exponential backoff)
    sub.read_exact(&mut header).await.unwrap();
    assert_eq!(header[0], 0x3A); // PUBLISH with DUP flag
    sub.read_exact(&mut vec![0u8; header[1] as usize]).await.unwrap();
    let second_retry = start.elapsed();
    info!("Second retry after: {:?}", second_retry);
    assert!(second_retry >= Duration::from_millis(3000) && second_retry < Duration::from_millis(5000));
    
    // Third retry should be around 7-10 seconds (exponential backoff)
    sub.read_exact(&mut header).await.unwrap();
    assert_eq!(header[0], 0x3A); // PUBLISH with DUP flag
    sub.read_exact(&mut vec![0u8; header[1] as usize]).await.unwrap();
    let third_retry = start.elapsed();
    info!("Third retry after: {:?}", third_retry);
    assert!(third_retry >= Duration::from_millis(7000) && third_retry < Duration::from_millis(10000));
