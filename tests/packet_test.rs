...
fn create_connect_packet(client_id: &str, clean_session: bool, keep_alive: u16) -> Vec<u8> {
    let mut packet = vec![
        ...
        (keep_alive >> 8) as u8, keep_alive as u8, // Keep alive
...
async fn read_packet_with_timeout(stream: &mut TcpStream, timeout_duration: Duration) -> Result<Vec<u8>, String> {
    // Read fixed header
...
async fn test_connect_with_client_id() {
    ...
    // Read CONNACK
    let response = read_packet_with_timeout(&mut stream, Duration::from_secs(2)).await.unwrap();
...
async fn test_subscribe_wildcard_topics() {
    ...
    stream.write_all(&subscribe_packet).await.unwrap();
    let _suback = read_packet_with_timeout(&mut stream, Duration::from_secs(2)).await.unwrap();
